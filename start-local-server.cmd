...
echo.
echo Site will open at http://127.0.0.1:%PORT%
echo Keep this window open while you are working.
echo.

start "" powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "$port = %PORT%; for ($i = 0; $i -lt 120; $i++) { if (Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue) { Start-Process ('http://127.0.0.1:' + $port); exit 0 }; Start-Sleep -Milliseconds 500 }"
