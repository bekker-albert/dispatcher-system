...
const catalogMeta: Record<DispatchReasonCatalogKind, { title: string; groupLabel: string; itemLabel: string }> = {
  downtime: {
...
export function AdminDispatchReasonCatalogs() {
  ...
  const deleteGroup = async (kind: DispatchReasonCatalogKind, groupId: string) => {
    ...
    if (!group) return;
    if (!window.confirm(`Удалить «${group.name}» и все значения второго уровня?`)) return;
  ...
  const addItem = async (kind: DispatchReasonCatalogKind, groupId: string) => {
    ...
    const next = {
      ...
      [kind]: catalogs[kind].map((group) => group.id === groupId
        ? { ...group, items: [...group.items, { id: createDispatchReasonCatalogId(`${kind}-item`), name }] }
        : group),
    };
          ...
          <section key={kind} style={catalogCardStyle}>
            <div style={catalogHeaderStyle}>
              <div>
                ...
                <div style={hintStyle}>{catalogMeta[kind].groupLabel} → {catalogMeta[kind].itemLabel}</div>
              </div>
            </div>
                  ...
                  <div style={itemsStyle}>
                    {group.items.map((item) => (
                      <div key={item.id} style={itemRowStyle}>
                        ...
                        />
                        <button type="button" onClick={() => void deleteItem(kind, group.id, item.id)} style={dangerIconButtonStyle} title="Удалить значение">
                    ...
                    <div style={itemRowStyle}>
                      ...
                      <input
                        ...
                        style={inputStyle}
                      />
                      <button type="button" onClick={() => void addItem(kind, group.id)} style={iconButtonStyle} title="Добавить">
...
const itemsStyle: CSSProperties = { display: "grid", gap: 6, paddingLeft: 10 };
