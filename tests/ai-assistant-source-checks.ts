...
assert.match(aiStateSource, /useState<typeof defaultAiAssistantDataset\.chatMessages>\(\[\]\)/);
assert.match(aiStateSource, /useState<typeof defaultAiAssistantDataset\.notifications>\(\[\]\)/);
assert.match(aiStateSource, /pagehide/);
assert.match(aiStateSource, /aiAssistantSystemNotificationEventName/);
assert.match(aiStateSource, /setNotifications\(\(current\) => upsertById/);
assert.match(aiStateSource, /status: "needs-approval"/);
assert.doesNotMatch(aiStateSource, /chat-system-\$\{detail\.id\}/);
assert.match(floatingDockSource, /dispatchAiAssistantSystemNotificationDecision/);
assert.match(floatingDockSource, /notification\.status === "needs-approval"/);
...
assert.match(floatingDockSource, /const \[isPinned, setIsPinned\] = useState\(false\)/);
assert.match(floatingDockSource, /panelRef = useRef<HTMLElement \| null>\(null\)/);
assert.match(floatingDockSource, /window\.addEventListener\("pointerdown", handlePointerDown\)/);
assert.match(floatingDockSource, /panel\.contains\(target\)/);
assert.match(floatingDockSource, /<Pin size=\{15\}/);
assert.match(floatingDockSource, /pinnedPanelHeaderButtonStyle/);
