...
assert.match(mysqlYearLoadSource, /EXISTS \([\s\S]*FROM pto_day_values AS values_for_year[\s\S]*values_for_year\.table_type = rows_for_year\.table_type[\s\S]*values_for_year\.row_id = rows_for_year\.row_id[\s\S]*values_for_year\.work_date >= \?[\s\S]*values_for_year\.work_date <= \?/);
