...
const wialonUnitsRouteSource = readSource("app/api/wialon/units/route.ts");
...
assert.match(wialonServiceSource, /const storedUnits = mappingsOnly[\s\S]*\? await listStoredWialonUnits\(\)[\s\S]*: await upsertWialonUnits/);
...
assert.match(wialonMysqlSource, /VALUES \(\?, \?, \?, \?, \?, NOW\(\)\)/);
assert.match(wialonMysqlSource, /synced_at = NOW\(\)/);
assert.match(wialonGuardSource, /canManageUsers/);
assert.match(wialonUnitsRouteSource, /checkWialonConnection/);
assert.match(wialonUnitsRouteSource, /source === "check"/);
...
assert.match(adminWialonSource, /\/api\/wialon\/units\?source=check/);
assert.doesNotMatch(adminWialonSource, /const checkConnection[\s\S]*setUnits\(body\.units \?\? \[\]\);[\s\S]*const syncUnits/);
assert.doesNotMatch(adminWialonSource, /\\u04[0-9a-fA-F]{2}/);
assert.match(adminWialonSource, /Проверить подключение/);
assert.match(adminWialonSource, /Загрузить технику/);
assert.match(adminWialonSource, /Сохранить сопоставления/);
assert.match(adminWialonSource, /Последняя синхронизация/);
assert.match(adminWialonSource, /Успешно/);
assert.match(adminWialonSource, /Ошибка/);
assert.match(adminWialonSource, /Не проверено/);
