...
assert.match(ptoDatabaseLoadRunnerSource, /import \{ hasInitialStoredPtoState, readInitialStoredPtoState \} from "@\/features\/app\/initialAppStorage";/);
assert.match(ptoDatabaseLoadRunnerSource, /const hydrateInitialLocalPtoState = \(\) => \{/);
assert.match(ptoDatabaseLoadRunnerSource, /buildInitialPtoState\(readInitialStoredPtoState\(\)\)/);
assert.match(ptoDatabaseLoadRunnerSource, /createPtoDatabaseState\(\{[\s\S]*manualYears: localInitialPtoState\.manualYears \?\? currentState\.manualYears/);
assert.match(ptoDatabaseLoadRunnerSource, /applyInitialPtoState\(localInitialPtoState/);
assert.match(ptoDatabaseLoadRunnerSource, /const hasLocalPtoSnapshot = hasStoredPtoStateRef\.current \|\| hasInitialStoredPtoState\(\);/);
assert.match(ptoDatabaseLoadRunnerSource, /hydrateInitialLocalPtoState\(\);[\s\S]*localPtoCanSkipFullDatabaseLoad/);
assert.match(ptoDatabaseLoadRunnerSource, /currentState: localPtoStateForResolution/);
assert.match(ptoDatabaseLoadRunnerSource, /normalizeLoadedPtoDatabaseState\(databaseState, localPtoStateForResolution/);
