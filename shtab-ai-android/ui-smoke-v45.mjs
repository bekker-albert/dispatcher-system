...
const gradle=fs.readFileSync(path.resolve('app/build.gradle.kts'),'utf8');
const quickActivity=fs.readFileSync(path.resolve('app/src/main/java/kz/shtabai/app/QuickCreateActivity.java'),'utf8');
const manifest=fs.readFileSync(path.resolve('app/src/main/AndroidManifest.xml'),'utf8');
const widgetLayout=fs.readFileSync(path.resolve('app/src/main/res/layout/shtab_widget_4x2.xml'),'utf8');
...
const fullSeed=`
  ...
  state.tasks.push({...__base,id:'assignment-today',kind:'assignment',title:'Поручение на сегодня',date:__t,endDate:__t,startTime:'12:00',time:'12:00',endTime:'12:30'});
  state.tasks.push({...__base,id:'event-today',kind:'event',title:'Событие на сегодня',date:__t,endDate:__t,startTime:'15:00',time:'15:00',endTime:'16:00'});
  state.tasks.push({...__base,id:'trip-today',kind:'trip',title:'Командировка на сегодня',date:__t,endDate:__t,startTime:'07:00',time:'07:00',endTime:'20:00'});
  state.tasks.push({...__base,id:'done-long',kind:'task',title:'Очень длинное название выполненной записи, которое обязано переноситься без сжатия и деформации',date:__y,endDate:__y,startTime:'09:00',time:'09:00',endTime:'10:00',status:'completed',completedDates:[__y]});
  state.tasks.push({...__base,id:'expired-final',kind:'task',title:'Истёкшая по последней дате',date:__y,endDate:__y,startTime:'08:00',time:'08:00',endTime:'08:30'});
  state.tasks.push({...__base,id:'future-final',kind:'task',title:'Не истекла до будущего окончания',date:__y,endDate:__p1,startTime:'08:00',time:'08:00',endTime:'18:00'});
  state.tasks.push({...__base,id:'daily-one',kind:'task',title:'Повторяющаяся запись без дублей',date:__t,endDate:__t,startTime:'17:00',time:'17:00',endTime:'17:30',recurrence:{type:'daily',interval:1}});
  state.notes.push({id:'note-today',title:'Рабочая заметка',body:'Текст заметки',tags:[],pinned:false,reminderDate:__p2,reminderTime:'13:00',updatedAt:Date.now(),createdAt:Date.now()});
