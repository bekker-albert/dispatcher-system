...
const scripts=Array.from({length:15},(_,i)=>fs.readFileSync(path.join(root,`chunk${i+1}.js`),'utf8')).join('\n;\n');
...
await run('task type and cross-month period',async()=>{
  ...
  assert(window.__shtabDiagnostics.taskIdsForDate('2026-08-01').includes(trip.id),'Period task missing inside cross-month range');
  assert(!window.__shtabDiagnostics.taskIdsForDate('2026-08-04').includes(trip.id),'Period task visible after end date');
});
