...
- Admin section rendering is behind `AppAdminContent`; database, vehicles, and report settings props now use smaller feature-owned adapters. Vehicle administration keeps its toolbar and autocomplete datalists in dedicated feature components so the main grid file can focus on table rendering.
