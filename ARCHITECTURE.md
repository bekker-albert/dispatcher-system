...
- Fleet operational state is date-keyed. Status, driver assignment, repair start date, repair reason, and daily note belong to `lib/domain/fleet/daily-state.ts` and future persistence keyed by `vehicleId + workDate`; do not add them as permanent `VehicleRow` card fields.
- Future fleet modules should be separated by business process: daily states, movements between projects/areas/locations, driver assignments, waybills, petitions, fuel issue statements, fuel reconciliation acts, and fuel acceptance/transfer acts. See `docs/fleet-operations-roadmap.md`.
