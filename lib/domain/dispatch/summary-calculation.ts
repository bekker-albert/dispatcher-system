...
import type { DispatchShift, DispatchSummaryRow } from "./summary-types";
...
export function normalizeDispatchSummaryRow(row: unknown, index: number, fallbackDate: string): DispatchSummaryRow | null {
  ...
  return {
    ...
    downtimeReasonGroup: stringValue(row.downtimeReasonGroup),
...
export function dispatchTotalHours(row: Pick<DispatchSummaryRow, "workHours" | "rentHours" | "repairHours" | "downtimeHours">) {
  return row.workHours + row.rentHours + row.repairHours + row.downtimeHours;
}
