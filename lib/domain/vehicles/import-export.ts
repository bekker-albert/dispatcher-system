...
export function createVehiclesFromImportTable(tableRows: string[][], defaultVehicleForm: VehicleRow) {
  ...
  const columns = {
    ...
    equipmentType: findTableColumn(headers, ["Тип техники", "Наименование техники", "Наименование"]),
    ...
    fuelCardNumber: findTableColumn(headers, ["№ топл.карты", "Номер топл.карты", "Топливная карта", "Fuel card"]),
    gpsInstalled: findTableColumn(headers, ["GPS", "GPS установлен", "GPS/СМТС"]),
    dutInstalled: findTableColumn(headers, ["ДУТ", "ДУТ установлен", "Датчик уровня топлива"]),
    canInstalled: findTableColumn(headers, ["CAN", "CAN установлен", "CAN-шина", "КАН"]),
    mainMileageSource: findTableColumn(headers, ["Источник пробега", "Пробег источник"]),
    mainFuelSource: findTableColumn(headers, ["Источник топлива", "Топливо источник"]),
    mainEngineHoursSource: findTableColumn(headers, ["Источник м/ч", "Источник моточасов", "Моточасы источник"]),
  ...
  return rows
    .map((row, index) => {
      ...
      const vehicle: VehicleRow = {
        ...
        plateNumber,
        garageNumber,
        fuelCardNumber: cell(row, columns.fuelCardNumber),
        gpsInstalled: cell(row, columns.gpsInstalled),
        dutInstalled: cell(row, columns.dutInstalled),
        canInstalled: cell(row, columns.canInstalled),
        mainMileageSource: cell(row, columns.mainMileageSource),
        mainFuelSource: cell(row, columns.mainFuelSource),
        mainEngineHoursSource: cell(row, columns.mainEngineHoursSource),
        vehicleType,
        equipmentType,
...
export function createVehicleExportRows(rows: VehicleRow[]) {
  return [
    ...
    ...rows.map((vehicle) => [
      ...
      vehicle.gpsInstalled ?? "",
      vehicle.dutInstalled ?? "",
      vehicle.canInstalled ?? "",
      vehicle.mainMileageSource ?? "",
      vehicle.mainFuelSource ?? "",
      vehicle.mainEngineHoursSource ?? "",
