...
function normalizeWialonTelemetry(unit: Record<string, unknown>, position: WialonPosition | null): WialonTelemetry {
  ...
  const rawFuelLevelSource = fuelFromLastMessage.value !== null
    ...
    : fuelFromParams.value !== null
      ...
      : null;
  const mileage = firstNumber(lastMessageParams.mileage, paramValue(params, "mileage"), unit.cnm, unit.cnm_km);
  const canMileage = firstNumber(lastMessageParams.can_mileage, paramValue(params, "can_mileage"));
  const engineHours = firstNumber(lastMessageParams.engine_hours, paramValue(params, "engine_hours"), unit.cneh);
